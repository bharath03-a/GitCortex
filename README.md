...
- **One smart dispatch tool by default**: `gcx` is a single compact MCP schema covering every graph operation, so agents don't pay the schema cost of all 27 individual tools on every turn — the fixed per-turn schema overhead of loading many tool definitions is a real, measured cost (see [`RELEASE-GATE.md`](docs/benchmarks/RELEASE-GATE.md)). All 27 individual tools remain one flag away (`gcx serve --full`) for power users and agents that want granular, single-purpose calls.
- **PR blast-radius bot**: `gcx init --ci` writes a GitHub Actions job that indexes on every pull request and posts a sticky PR comment with risk-scored blast radius — changed nodes, affected callers, and a 🟢🟡🟠🔴 risk band — via `gcx blast-radius --format github-comment`. No hosted service, no waitlist; it runs in your own CI today ([full section below](#ci--pr-blast-radius-bot)). Example comment output:

  ```
  ## 🟠 Blast Radius — Risk: **HIGH**

  > `feat/auth` → `main` · 4 changed · 18 affected

  ### Changed nodes
  | Kind | Name | File |
  |------|------|------|
  | `function` | `hash_password` | `src/auth.rs:42` |

  ### Affected callers
  | Hop | Kind | Name | File |
  |-----|------|------|------|
  | 1 | `function` | `login` | `src/handlers.rs:88` |
...
Every binary artifact on a GitHub Release is signed with [cosign](https://github.com/sigstore/cosign)
using keyless OIDC (Sigstore) signing — no private key is stored anywhere, the signature is tied to
the GitHub Actions run that built it. Each artifact ships with a `.sig` and `.pem` alongside it.

```bash
# download the artifact, its signature, and its certificate
curl -LO https://github.com/bharath03-a/GitCortex/releases/latest/download/gitcortex-aarch64-apple-darwin.tar.xz
curl -LO https://github.com/bharath03-a/GitCortex/releases/latest/download/gitcortex-aarch64-apple-darwin.tar.xz.sig
curl -LO https://github.com/bharath03-a/GitCortex/releases/latest/download/gitcortex-aarch64-apple-darwin.tar.xz.pem

cosign verify-blob \
  --certificate gitcortex-aarch64-apple-darwin.tar.xz.pem \
  --signature gitcortex-aarch64-apple-darwin.tar.xz.sig \
  --certificate-identity-regexp 'https://github\.com/bharath03-a/GitCortex/\.github/workflows/release\.yml@.*' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  gitcortex-aarch64-apple-darwin.tar.xz
```

A successful verification prints `Verified OK`. Substitute the artifact filename for your platform.
...
gcx doctor
  ...
  [ok] gcx v0.7.5 on PATH (/usr/local/bin/gcx)
